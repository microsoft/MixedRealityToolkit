...
class DoubleElementImpl : public DoubleElement
{
public:
	DoubleElementImpl(SyncContext* syncContext, const XStringPtr& name, XGuid id, const XValue& startingValue);

	// DoubleElement Functions:

	// Get the current value of the element.  Returns immediately, does not allocate.  
	virtual double GetValue() const XTOVERRIDE;
	...
	virtual void SetValue(double newValue) XTOVERRIDE;
