...
class StringElementImpl : public StringElement
{
public:
	StringElementImpl(SyncContext* context, const XStringPtr& name, XGuid id, const XValue& startingValue);

	// StringElement Functions:

	// Get the current value of the element.  Returns immediately, does not allocate.  
	virtual const XStringPtr& GetValue() const XTOVERRIDE;
	...
	virtual void SetValue(const XStringPtr& newValue) XTOVERRIDE;
	...
	// Local Functions:
	void SetLocal(const XStringPtr& newValue);
