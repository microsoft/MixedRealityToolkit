...
// FloatElementImpl.h
//
// Implementation of the FloatElement interface
//
...
class FloatElementImpl : public FloatElement
{
public:
	FloatElementImpl(SyncContext* syncContext, const XStringPtr& name, XGuid id, const XValue& startingValue);

	// FloatElement Functions:

	// Get the current value of the element.  Returns immediately, does not allocate.  
	virtual float GetValue() const XTOVERRIDE;
	...
	virtual void SetValue(float newValue) XTOVERRIDE;
