...
class IntArrayElementImpl : public IntArrayElement, public ArrayElement
{
	XTOOLS_REFLECTION_DECLARE(IntArrayElementImpl)

public:
	typedef ListenerList<IntArrayListener> ListenerList;
	DECLARE_PTR(ListenerList);

	IntArrayElementImpl(SyncContext* syncContext, const XStringPtr& name, XGuid id, const XValue& startingValue);
	...
	virtual int32 GetValue(int32 index) const XTOVERRIDE;

	/// Sets the value of the element at the given index.  Returns immediately, does not allocate
	virtual void SetValue(int32 index, int32 newValue) XTOVERRIDE;

	/// Inserts the given value into the array at the given index
	virtual void InsertValue(int32 index, int value) XTOVERRIDE;
	...
	virtual void AddListener(IntArrayListener* newListener) XTOVERRIDE;
	...
	virtual void RemoveListener(IntArrayListener* oldListener) XTOVERRIDE;
