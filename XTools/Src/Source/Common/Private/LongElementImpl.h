...
class LongElementImpl : public LongElement
{
public:
	LongElementImpl(SyncContext* syncContext, const XStringPtr& name, XGuid id, const XValue& startingValue);

	// LongElement Functions:

	// Get the current value of the element.  Returns immediately, does not allocate.  
	virtual int64 GetValue() const XTOVERRIDE;
	...
	virtual void SetValue(int64 newValue) XTOVERRIDE;
