...
public:
	...
	virtual void OnRoomCreated(const RoomPtr& newRoom) {}
	...
	virtual void OnUserJoinedRoom(const RoomPtr& room, const UserPtr& user) {}

	/// Notification that a User has left a Room.  Includes the local user
	virtual void OnUserLeftRoom(const RoomPtr& room, const UserPtr& user) {}
