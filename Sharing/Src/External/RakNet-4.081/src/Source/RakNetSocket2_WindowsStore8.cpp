...
task<RNS2BindResult> DoBind(DatagramSocket^ listener, Platform::String ^localServiceName)
{
	return (task<void>(listener->BindServiceNameAsync(localServiceName)).then([](task<void> previousTask)
	{
		try
		{
...
RNS2BindResult RNS2_WindowsStore8::Bind( Platform::String ^localServiceName ) 
{
	...
	auto bindResult = DoBind(listener, localServiceName);
...
//	task<void> bindOp(listener->BindServiceNameAsync(ref new Platform::String()));
	...
	op->Completed = ref new AsyncOperationCompletedHandler< Windows::Foundation::Collections::IVectorView<Windows::Networking::EndpointPair^>^ >(
		...
		/// MICROSOFT PROJECT B CHANGES END
			...
			{
				HostName^ remoteHostName = result2->GetAt(i)->RemoteHostName;
				if(remoteHostName->Type == HostNameType::Ipv4)
				{
			/// MICROSOFT PROJECT B CHANGES END
					Platform::String ^name = remoteHostName->DisplayName;
					RakString rs2;
