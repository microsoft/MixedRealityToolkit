...
StartupResult RakPeer::Startup( unsigned int maxConnections, SocketDescriptor *socketDescriptors, unsigned socketDescriptorCount, /* Microsoft Project B Changes Begin*/ RakNetSmartPtr<SignaledEvent> networkThreadWaitEvent, /* Microsoft Project B Changes End*/ int threadPriority)
{
	...
	for (i=0; i<socketDescriptorCount; i++)
	{
		...
		*/
		...
		Platform::String^ bindString;
		if (socketDescriptors[i].port != 0)
		{
			char portStr[32];
			Itoa(socketDescriptors[i].port, portStr, 10);
			RakNet::RakString rs(portStr);
			WCHAR *w_char = rs.ToWideChar();

			bindString = ref new Platform::String(w_char);
		}
		else
		{
			bindString = ref new Platform::String();
		}

		br = ((RNS2_WindowsStore8*)r2)->Bind(bindString);
		if (br!=BR_SUCCESS)
		{
...
//					SocketLayer::SetDoNotFragment(socketToUse, 1);
					...
					RNS2SendResult result = 0;
					try
					{
						result = socketToUse->Send(&bsp, _FILE_AND_LINE_);
					}
					catch (...)
					{
