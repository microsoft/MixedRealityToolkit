...
class StringArrayElementImpl : public StringArrayElement, public ArrayElement
{
	XTOOLS_REFLECTION_DECLARE(StringArrayElementImpl)

public:
	typedef ListenerList<StringArrayListener> ListenerList;
	DECLARE_PTR(ListenerList);

	StringArrayElementImpl(SyncContext* syncContext, const XStringPtr& name, XGuid id, const XValue& startingValue);
	...
	virtual XStringPtr GetValue(int32 index) const XTOVERRIDE;

	/// Sets the value of the element at the given index.  Returns immediately, does not allocate
	virtual void SetValue(int32 index, const XStringPtr& newValue) XTOVERRIDE;

	/// Inserts the given value into the array at the given index
	virtual void InsertValue(int32 index, const XStringPtr& value) XTOVERRIDE;
	...
	virtual void AddListener(StringArrayListener* newListener) XTOVERRIDE;
	...
	virtual void RemoveListener(StringArrayListener* oldListener) XTOVERRIDE;
...
private:
	...
	XStringPtr			m_name;
	XGuid				m_guid;
	std::vector<XStringPtr>	m_values;
