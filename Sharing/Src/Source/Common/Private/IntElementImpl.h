...
// IntElementImpl.h
//
// Implementation of the IntElement interface
//
...
class IntElementImpl : public IntElement
{
public:
	IntElementImpl(SyncContext* syncContext, const XStringPtr& name, XGuid id, const XValue& startingValue);

	// IntElement Functions:

	// Get the current value of the element.  Returns immediately, does not allocate.  
	virtual int32 GetValue() const XTOVERRIDE;
	...
	virtual void SetValue(int32 newValue) XTOVERRIDE;
